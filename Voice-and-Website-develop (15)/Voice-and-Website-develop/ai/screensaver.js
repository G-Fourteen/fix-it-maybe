document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchDynamicPromptWithRetry(maxRetries = 3, delayMs = 2000) {
        ...
        const body = { messages, model: "unity", nonce: Date.now().toString() + Math.random().toString(36).substring(2) };
        console.log("Sending API request for new prompt:", JSON.stringify(body));

        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                const response = await fetch(`https://text.pollinations.ai/openai?seed=${seed}`, {
                    method: "POST",
