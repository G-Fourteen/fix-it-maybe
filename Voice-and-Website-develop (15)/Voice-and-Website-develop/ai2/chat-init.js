document.addEventListener("DOMContentLoaded", () => {
    const { chatBox, chatInput, clearChatBtn, voiceToggleBtn, modelSelect, synth, autoSpeakEnabled, speakMessage, stopSpeaking, showToast, toggleSpeechRecognition, initSpeechRecognition, processAIInstructions, handleVoiceCommand, executeCommand } = window._chatInternals;
    const imagePatterns = [
    ...
    window.sendToPollinations = (callback = null, overrideContent = null) => {
        ...
        fetch(apiUrl, {
            ...
            .catch(err => {
                ...
                if (callback) callback();
                const btn = window._chatInternals?.sendButton;
                const input = window._chatInternals?.chatInput;
